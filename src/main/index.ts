...
import setupModule from "../transcription/setup";
...
console.log("[main/index.ts] Imported setupModule:", setupModule);
...
let CAPTURE_PAGE_DIR: string;

// Determine the appropriate path in development or production
if (process.env.NODE_ENV === "development") {
  CAPTURE_PAGE_DIR = join(__dirname, "../../src/capture-page");
} else {
  CAPTURE_PAGE_DIR = join(__dirname, "../capture-page");
}
...
function generateAudioFilePaths(): {
  ...
  try {
    ...
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const systemAudioPath = join(tempDir, `system-audio-${timestamp}.m4a`);
    const micAudioPath = join(tempDir, `mic-audio-${timestamp}.m4a`);
    ...
    return {
      systemAudioPath: join(os.tmpdir(), `system-audio-${Date.now()}.m4a`),
      micAudioPath: join(os.tmpdir(), `mic-audio-${Date.now()}.m4a`),
    };
  }
}
...
ipcMain.handle(
  ...
  async (event: IpcMainInvokeEvent, eventId: string) => {
    try {
      console.log("Loading meeting note for event:", eventId);
      const note = loadMeetingNote(eventId);
      console.log("Meeting note loaded, content length:", note?.length || 0);
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      // Update current settings to include the LLM settings
      const currentSettings = loadAllSettings();
